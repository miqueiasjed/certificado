...
<body>
    ...
    <div class="section">
        ...
        <p><strong>CONTRATADA:</strong> {{ $company['name'] }}, pessoa jurídica de direito privado, inscrita no CNPJ
        n° {{ $company['cnpj'] }}, com sede à {{ $company['address'] }}, {{ $company['city'] }}, neste ato devidamente
        representada por {{ $company['legal_representative'] }}, portador do CPF n° {{ $company['legal_representative_cpf'] }} e RG n° {{ $company['legal_representative_rg'] }}, doravante
        denominada CONTRATADA;</p>
    </div>
    ...
    <div class="section">
        ...
        {{ $address->street }}, {{ $address->number }} - {{ $address->district }}, {{ $address->city }}/{{ $address->state }} - CEP: {{ $address->zip }}, doravante denominado CONTRATANTE;</p>
    </div>
    ...
    <div class="clause">
        ...
        CONTRATANTE, localizado em {{ $address->street }}, {{ $address->number }} - {{ $address->district }}, {{ $address->city }}/{{ $address->state }}. Os serviços visam combater e
        controlar pragas como insetos (baratas, formigas, mosquitos, etc.), aracnídeos, roedores e demais
        vetores especificados no escopo abaixo, mediante aplicação de técnicas e produtos adequados.</p>
    </div>
    ...
    <div class="clause">
        ...
        {{ $contract->start_date ? $contract->start_date->format('d/m/Y') : '____/____/____' }}, abrangendo o tratamento das pragas indicadas neste contrato. Após a realização dessa
        visita e eventuais retornos dentro da garantia, o contrato será considerado cumprido.</p>
        ...
        @else
            [___] meses
        @endif
        , com início em {{ $contract->start_date ? $contract->start_date->format('d/m/Y') : '____/____/____' }} e término previsto em {{ $contract->end_date ? $contract->end_date->format('d/m/Y') : '____/____/____' }}. O cronograma de visitas
        deverá ser acordado entre as Partes e poderá constar em anexo a este contrato. Cada visita
        realizada será registrada em relatório/certificado individual.</p>
    </div>
    ...
    <div class="clause">
        ...
        @else
            [listar as pragas e/ou pragas comuns abrangidas, por exemplo: baratas, formigas, mosquitos, ratos, cupins, etc.]
        @endif
        . Qualquer extensão do
        escopo para pragas não listadas deverá ser objeto de aditivo ou contrato em separado.</p>
    </div>
    ...
    <div class="clause">
        <p><strong>2.4. Prazo de Realização:</strong> A prestação do serviço deverá ocorrer na data e horário agendados entre
    ...
    <div class="clause">
        <p><strong>3.1. Acesso ao Local:</strong> O CONTRATANTE deverá disponibilizar o imóvel e proporcionar livre acesso
    ...
    <div class="clause">
        ...
        Contrato de Visita Única: Este contrato tem vigência início na data de sua assinatura e será considerado cumprido e encerrado após a realização do serviço descrito na Cláusula 1ª e o decurso do prazo de garantia correspondente.
        @else
        Contrato Periódico: O presente contrato vigorará pelo prazo estipulado, iniciando-se em {{ $contract->start_date ? $contract->start_date->format('d/m/Y') : '____/____/____' }} e terminando em {{ $contract->end_date ? $contract->end_date->format('d/m/Y') : '____/____/____' }}.
        @endif
        </p>
    </div>

    <div class="clause">
        <p><strong>6.2. Rescisão Antecipada:</strong> Qualquer das Partes poderá rescindir este contrato antes do término
    ...
    <div class="clause">
        <p><strong>7.1. Alterações no Contrato:</strong> Qualquer modificação nas condições aqui pactuadas deverá ser feita mediante acordo escrito entre as Partes, por meio de termo aditivo a este contrato.</p>
    </div>

    <div class="clause">
        <p><strong>7.2. Integralidade:</strong> Este contrato representa o acordo integral entre as Partes sobre seu objeto,
        substituindo quaisquer entendimentos ou negociações anteriores, verbais ou escritas.</p>
    </div>

    <div class="clause">
        <p><strong>7.3. Relação de Consumo:</strong> As Partes reconhecem que esta relação contratual caracteriza relação de
    ...
    <div class="clause">
        <p><strong>8.1.</strong> Para dirimir quaisquer dúvidas ou litígios oriundos deste contrato, as Partes elegem de comum
        acordo o Foro da Comarca de Fortaleza, Estado do Ceará, como o competente para solucionar a
