<template>
  <AuthenticatedLayout>
    <PageHeader
      title="Editar Evento de Dispositivo"
      subtitle="Modifique as informações do evento"
    />
      ...
      <Card>
        <form @submit.prevent="submit" class="p-6 space-y-6">
          ...
          <div class="flex justify-end space-x-3">
            <Link :href="route('device-events.show', deviceEvent.id)" class="btn-secondary">
              Cancelar
            </Link>
            <button
              type="submit"
            ...
            >
              ...
              {{ form.processing ? 'Salvando...' : 'Atualizar Evento' }}
            </button>
          </div>
        </form>
      </Card>
...
const form = useForm({
  ...
  cleaning_notes: props.deviceEvent?.cleaning_notes || '',
  bait_change_type: props.deviceEvent?.bait_change_type || '',
  bait_change_lot: props.deviceEvent?.bait_change_lot || '',
  bait_change_quantity: props.deviceEvent?.bait_change_quantity || '',
  technician_notes: props.deviceEvent?.technician_notes || '',
  active: props.deviceEvent?.active !== undefined ? props.deviceEvent.active : true,
});
