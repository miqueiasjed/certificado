<template>
  <AuthenticatedLayout>
    <PageHeader
      title="Editar Avistamento de Praga"
      subtitle="Modifique as informações do avistamento"
    />
      ...
      <Card>
        <form @submit.prevent="submit" class="p-6 space-y-6">
          ...
          <div class="flex justify-end space-x-3">
            <Link :href="route('pest-sightings.show', pestSighting.id)" class="btn-secondary">
              Cancelar
            </Link>
            <button
              type="submit"
            ...
            >
              ...
              {{ form.processing ? 'Salvando...' : 'Atualizar Avistamento' }}
            </button>
          </div>
        </form>
      </Card>
...
const form = useForm({
  ...
  environmental_conditions: props.pestSighting?.environmental_conditions || '',
  control_measures_applied: props.pestSighting?.control_measures_applied || '',
  technician_notes: props.pestSighting?.technician_notes || '',
  active: props.pestSighting?.active !== undefined ? props.pestSighting.active : true,
});
