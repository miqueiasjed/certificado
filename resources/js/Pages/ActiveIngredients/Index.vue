  ...
  <AuthenticatedLayout>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
      <div class="flex-1 min-w-0">
        <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl sm:truncate">Princípios Ativos</h1>
        <p class="mt-1 text-sm text-gray-600 sm:text-base">Gerencie todos os princípios ativos cadastrados no sistema</p>
      </div>
      <div class="flex flex-col gap-2 sm:flex-row sm:gap-3">
        <Link
          href="/active-ingredients/create"
    ...
    <Card>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-if="loading || !activeIngredients" class="animate-pulse">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                Carregando...
              </td>
            </tr>
            <tr v-else-if="activeIngredients && activeIngredients.data && activeIngredients.data.length === 0">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                Nenhum princípio ativo encontrado
              </td>
            </tr>
            <tr v-else v-for="activeIngredient in (activeIngredients?.data || [])" :key="activeIngredient.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                      :class="activeIngredient.products_count > 0 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'">
                  {{ activeIngredient.products_count || 0 }} produto(s)
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ formatarData(activeIngredient.created_at) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end space-x-3">
                  <Link
                    :href="`/active-ingredients/${activeIngredient.id}`"
                  ...
                  <Link
                    :href="`/active-ingredients/${activeIngredient.id}/edit`"
                  ...
                  <button
                    @click="deleteActiveIngredient(activeIngredient)"
                    :disabled="activeIngredient.products_count > 0"
                    :class="activeIngredient.products_count > 0
                      ? 'text-gray-400 cursor-not-allowed'
                      : 'text-red-600 hover:text-red-900'"
                    :title="activeIngredient.products_count > 0
      ...
      <!-- Paginação -->
      <Pagination
        v-if="activeIngredients && activeIngredients.last_page > 1"
