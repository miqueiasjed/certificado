<template>
  <AuthenticatedLayout>
    <template #header>
      <PageHeader title="Editar Técnico" :description="`Editando: ${technician.name}`">
        <template #actions>
          <Link :href="`/technicians/${technician.id}`" class="btn-secondary">
...
const form = useForm({
  ...
  registration_number: props.technician.registration_number || '',
  is_active: props.technician.is_active || false,
...
const submitForm = () => {
  form.put(`/technicians/${props.technician.id}`, {
    onSuccess: () => {
      // Sucesso
    },
  });
